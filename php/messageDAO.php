<?php
    ...
    function getMessagesBetweenPeople(PDO $pdo, string $personOne, string $personTwo) : array | null {
        $stmt = prepare($pdo, "SELECT * FROM message WHERE sender IN (:personOne, :personTwo) AND receiver IN (:personOne, :personTwo) ORDER BY sentDate ASC, sentHour ASC");
        execute($stmt, [":personOne" => $personOne, ":personTwo" => $personTwo]);

        $messages = array();
        while ($message = $stmt->fetch()) {
            $messages[] = new Message($message["sender"], $message["receiver"], new DateTime($message["sentDate"]), $message["sentHour"], $message["content"], $message["liked"]);
        }
